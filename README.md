...
- **103 Tests, All Passing**: 18 UI tests × 3 browsers (Chromium, Firefox, WebKit) = 54 UI runs + 49 API-level tests. The UI suite (`TalentX-Frontend/tests/`) covers every meaningful user-facing flow across 11 spec files. The API suite (`e2e/specs/`) covers backend contracts across 4 spec files with no browser overhead.
- **Interview Scheduling Coverage**: `scheduling.spec.js` (6 tests) verifies auth/RBAC/ownership checks (scheduling for a job you don't own returns 403), empty-timeslot validation, 404 for a nonexistent application, and the happy path that marks an application `"interviewing"` — the endpoint had zero coverage despite having a real status-machine transition and a job-ownership authorization check.
- **AI Match Bulk Coverage**: 4 tests in `ai.spec.js` cover `/ai/match-bulk` — the endpoint that powers both the talent job feed and the employer matched-talents view on every dashboard load. Includes an order-preservation assertion critical for correct score-to-talent mapping.
- **Real-Time Notification Testing**: Two-browser Playwright tests verify that WebSocket notifications arrive live without page refresh — employer schedules an interview or sends an invitation, and the talent's bell icon pulses with the correct notification. These tests uncovered two production bugs: a `RealtimeProvider` that never connected after login (Entry L), and a header-mode auth middleware that silently created duplicate users causing notification delivery to the wrong Socket.IO room (Entry M).
- **Invitation Lifecycle (UI)**: `invitation-lifecycle.spec.js` tests the full talent-side UI flow — employer invites a talent via API, talent opens the Invitations tab, sees the job title with Accept/Decline buttons, responds, and verifies the toast and status badge update.
- **API-Level Testing**: A dedicated `api` Playwright project hits the Express backend directly (auth requirements, RBAC, CORS, rate limiting with per-user identity keying, input validation, invitations state machine, scheduling ownership, error-response hygiene) with no browser overhead, catching gaps — like a rate limiter that was imported but never wired up — that UI clicks alone can't reach. See [QA-STRATEGY.md](./QA-STRATEGY.md) Entry F.
...
- **Multi-Actor Testing**: Seamlessly boots up multiple isolated browser contexts in a single test to simulate real-time WebSocket messaging and notification delivery between an Employer browser and a Talent browser simultaneously.
...
- **CI/CD Integration**: Fully integrated into GitHub Actions (`.github/workflows/playwright.yml`), orchestrating a Postgres service container, full-stack background server bootups, all three browser projects (Chromium, Firefox, WebKit) plus the API project, and automated HTML report generation for every Pull Request.
- **No Unmaintained Suites**: A second, earlier-generation UI test suite (root-level `e2e/specs/`) existed alongside this one but had never actually run in CI and had drifted from the current app. It was retired rather than left in the repo — see [QA-STRATEGY.md](./QA-STRATEGY.md) Entry G. `e2e/specs/ai.spec.js`, `security.spec.js`, `invitations.spec.js`, and `scheduling.spec.js` survived or were added fresh and power the `api` project above.
