...
- **100% Pass Rate**: Last confirmed run: 30/30 UI tests (Chromium/Firefox) and 37/37 API-level tests, including Invitations lifecycle coverage ([QA-STRATEGY.md](./QA-STRATEGY.md) Entry H). A few changes are pending reconfirmation since then — a low-value UI test removed, a new API test added, WebKit wired in as a third browser — see QA-STRATEGY.md for the current honest state rather than a running tally here.
